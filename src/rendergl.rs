...
impl TextureProgram {
    ...
    fn bind_uniforms_and_attributes(&self,
        ...
        let new_coords: [f32, ..8] = [
            ...
            unit_rect.origin.x, unit_rect.size.height,
            unit_rect.size.width, unit_rect.origin.y,
            unit_rect.size.width, unit_rect.size.height,
        ];
        ...
        Some(clip_rect) => {
            match clip_rect.intersection(&rect) {
                Some(intersected_rect) => {
                    let point = intersected_rect.origin - rect.origin;
                    Rect(Point2D(point.x / rect.size.width, point.y / rect.size.height),
                         Size2D(intersected_rect.size.width / rect.size.width,
                                intersected_rect.size.height / rect.size.height))
    ...
    fn render(&self,
        ...
        if *self.masks_to_bounds.borrow() {
            clip_rect = match clip_rect {
                Some(clip_rect) => clip_rect.intersection(&bounds),
                None => Some(bounds),
            };
        ...
        match clip_rect {
            Some(ref mut clip_rect) => clip_rect.origin = clip_rect.origin - bounds.origin,
            None => {},
        }

        self.create_textures(&render_context.compositing_context);
        self.do_for_all_tiles(|tile: &Tile| {
            tile.render(render_context, tile_transform, scene_size, clip_rect)
        });
        ...
        for child in self.children().iter() {
            child.render(render_context, cumulative_transform, scene_size, clip_rect)
        }
...
impl Render for Tile {
    fn render(&self,
        ...
        bind_and_render_quad(render_context,
                             ...
                             &transform,
                             scene_size,
                             map_clip_to_unit_rectangle(self.bounds, clip_rect));
...
pub fn render_scene<T>(root_layer: Rc<Layer<T>>,
    ...
    root_layer.render(render_context, transform, scene.size, None);
}
